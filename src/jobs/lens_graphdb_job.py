...
LastEditTime: 2024-10-24 16:54:43
...
class LensGraphDB(object):
    ...
    def delete_before_loading_lens(self):
        '''
        description: 
        ...
        if response.status_code != 200:
            error_msg = "graphdb_server delete_before_loading_lens failed: url={}, {} {}".format(
        ...
        if "error" in res:
            if res["error"] is True:
                error_msg = "graphdb_server delete_before_loading_lens failed: url={}, error={}".format(
                    delete_url, res)
                logging.error(error_msg)
                raise Exception(error_msg)
            else:
                ...
                ts_delta = end - beg
                logging.info("graphdb_server delete_before_loading_lens succeeded, cost: %d", ts_delta)
    ...
    def dumps_to_graphdb(self):
        try:
            ...
            self.save_graph_id()
            self.delete_before_loading_lens()
